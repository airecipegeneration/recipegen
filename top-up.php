        ...
        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md" data-aos="fade-up" data-aos-delay="100">
            <div class="bg-white py-8 px-4 shadow-lg sm:rounded-lg sm:px-10">
                <div class="mb-6 p-3 bg-green-50 border border-green-200 rounded-md text-center">
                    <p class="text-sm text-green-800">Rate: <strong><?php echo $currency_config['symbol']; ?>1.00
                            <?php echo $current_currency; ?> ≈ <?php echo floor(100 / $currency_config['rate']); ?>
                            Tokens</strong></p>
                </div>
                <form id="topup-form" class="space-y-6">
                    <div>
                        ...
                        <div id="preset-amounts" class="mt-2 grid grid-cols-2 gap-4">
                            <button type="button" class="preset-btn"
                                data-amount="<?php echo number_format(calculate_price_for_tokens(1000, $current_currency), 2, '.', ''); ?>"><?php echo format_price(calculate_price_for_tokens(1000, $current_currency), $current_currency); ?>
                                <span class="block text-xs font-normal">1,000 Tokens</span></button>
                            <button type="button" class="preset-btn"
                                data-amount="<?php echo number_format(calculate_price_for_tokens(2750, $current_currency), 2, '.', ''); ?>"><?php echo format_price(calculate_price_for_tokens(2750, $current_currency), $current_currency); ?>
                                <span class="block text-xs font-normal">2,750 Tokens</span></button>
                            <button type="button" class="preset-btn"
                                data-amount="<?php echo number_format(calculate_price_for_tokens(6000, $current_currency), 2, '.', ''); ?>"><?php echo format_price(calculate_price_for_tokens(6000, $current_currency), $current_currency); ?>
                                <span class="block text-xs font-normal">6,000 Tokens</span></button>
                            <button type="button" class="preset-btn" data-amount="custom">Custom</button>
                        </div>
                    </div>
                    <div>
                        <label for="topup-amount" class="block text-sm font-medium text-text-main">Or enter custom
                            amount</label>
                        <div class="mt-1 relative rounded-md shadow-sm">
                            ...
                            <input type="text" id="topup-amount" name="amount" required inputmode="decimal"
                                placeholder="15.00"
                                class="focus:ring-primary focus:border-primary block w-full pl-7 pr-12 sm:text-sm border-border rounded-md"
                                data-currency="<?php echo $current_currency; ?>"
                                data-rate="<?php echo $currency_config['rate']; ?>">
                        </div>
                    ...
                    <div class="space-y-4">
                        <button type="submit" id="checkout-button"
                            class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary-hover">
                            Add Tokens
                        </button>
                    </div>
                </form>
